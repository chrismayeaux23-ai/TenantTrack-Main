...
export async function registerRoutes(
  ...
  app.post('/api/vendors/discover', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const user = await authStorage.getUser(userId);
      const tier = user?.subscriptionTier || "trial";
      if (tier !== "growth" && tier !== "pro") {
        return res.status(403).json({ message: "Vendor discovery requires Growth or Pro plan" });
      }
      ...
      if (!(TRADE_CATEGORIES as readonly string[]).includes(input.tradeCategory)) {
        return res.status(400).json({ message: "Invalid trade category" });
      }

      const { discoverVendorsFromPlaces, normalizeVendorKey } = await import("./vendorDiscovery");
      const results = await discoverVendorsFromPlaces(input);

      const existingVendors = await storage.getVendorsByLandlord(userId);
      const existingByKey = new Set(existingVendors.map(v => normalizeVendorKey(v.name, v.phone)));
      const existingByExtId = new Set(
        existingVendors.map(v => v.externalSourceId).filter((id): id is string => !!id)
      );

      const enriched = results.map(r => ({
        ...r,
        alreadyInNetwork:
          existingByExtId.has(r.externalSourceId) ||
          existingByKey.has(normalizeVendorKey(r.name, r.phone)),
      }));

      res.json({ results: enriched });
    } catch (err: any) {
      if (err instanceof z.ZodError) return res.status(400).json({ message: err.errors[0].message });
      const msg = err?.message || "Failed to search for vendors";
      console.error("Vendor discovery error:", msg);
      res.status(500).json({ message: msg });
    }
  });

  app.post('/api/vendors/discover/add', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const user = await authStorage.getUser(userId);
      const tier = user?.subscriptionTier || "trial";
      ...
      const input = z.object({
        ...
        externalRating: z.number().int().nullable().optional(),
        externalReviewCount: z.number().int().nullable().optional(),
        externalSourceUrl: z.string().max(500).nullable().optional(),
        seedTrustScore: z.number().int().min(0).max(100),
      ...
      const { normalizeVendorKey } = await import("./vendorDiscovery");
      const existingVendors = await storage.getVendorsByLandlord(userId);
      const dupByExt = existingVendors.find(v => v.externalSourceId === input.externalSourceId);
      const dupByKey = existingVendors.find(v => normalizeVendorKey(v.name, v.phone) === normalizeVendorKey(input.name, input.phone || null));
      if (dupByExt || dupByKey) {
        return res.status(409).json({ message: "This vendor is already in your network" });
      }

      const ratingNote = input.externalRating !== null && input.externalRating !== undefined && input.externalReviewCount
        ? `Imported from public review data — ${(input.externalRating / 10).toFixed(1)} ★ (${input.externalReviewCount} reviews).`
        : `Imported from public business directory.`;
      const addressNote = input.address ? ` Address: ${input.address}.` : "";
      ...
      const vendor = await storage.createVendor({
        ...
        notes: ratingNote + addressNote,
        status: "active",
        source: "discovery",
        externalRating: input.externalRating ?? null,
        externalReviewCount: input.externalReviewCount ?? null,
        externalSourceId: input.externalSourceId,
        externalSourceUrl: input.externalSourceUrl ?? null,
        seedTrustScore: input.seedTrustScore,
      } as any);
